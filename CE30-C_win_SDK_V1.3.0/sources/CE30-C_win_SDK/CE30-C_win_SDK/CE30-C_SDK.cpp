...
bool TCP_connect(SOCKET _host, char *_ip)
{
	...
	fd_set set;
	unsigned long ul = 1;
	ioctlsocket(_host, FIONBIO, &ul);
	bool ret = false;

	// connect server
	if (connect(_host, (LPSOCKADDR)&server_addr, sizeof(server_addr)) == -1)
	{
		tm.tv_sec = 7;
		tm.tv_usec = 0;
		FD_ZERO(&set);
		FD_SET(_host, &set);
		if (select(_host+1, NULL, &set, NULL, &tm) > 0)
		{
			getsockopt(_host, SOL_SOCKET, SO_ERROR, (char *)&error, &len);
			if (error == 0)
			{
				ret = true;
			}
	...
	{
		int iTimeOut = 3000;
		setsockopt(_host, SOL_SOCKET, SO_RCVTIMEO, (char*)&iTimeOut, sizeof(iTimeOut));
		setsockopt(_host, SOL_SOCKET, SO_SNDTIMEO, (char*)&iTimeOut, sizeof(iTimeOut));
		return true;
	}
}
...
// clear socket buffer
void clearSocketBuffer(SOCKET _host)
{
...
namespace benewake
{
	...
	__declspec(dllexport) bool __stdcall startMeasurement(SOCKET &_host, int _times = 0)
	{
		clearSocketBuffer(_host);
		...
		{
			char times[9];
			sprintf_s(times, "%d", _times);
			char startNTimes[51] = "getDistanceAndAmplitudeSortedTimes ";
			strcat_s(startNTimes, times);
			int fillSpaceNum = 50 - strlen(startNTimes);
			for (int i = 0; i < fillSpaceNum; i++)
			{
				strcat_s(startNTimes, " ");
			}
			if (send_command(_host, startNTimes, strlen(startNTimes)) < 0)
			{
		...
		{
			printf("ERROR: Measurement times value must be natural number!\n");
			return false;
		}
	}
	...
	__declspec(dllexport) bool __stdcall stopMeasurement(SOCKET _host)
	{
